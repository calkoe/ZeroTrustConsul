...
resource "docker_container" "server" {
  ...
  upload {
    file = "cert.pem"
    source = "merkur-server-consul-0.pem"
    source_hash = filemd5("merkur-server-consul-0.pem")
  }
  upload {
    file = "key.pem"
    source = "merkur-server-consul-0-key.pem"
    source_hash = filemd5("merkur-server-consul-0-key.pem")
  }
  volumes {
